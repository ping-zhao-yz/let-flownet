...
__all__ = ['let_flownet_evtcount']


class Let_Flownet_EvtCount(BaseModel):
    expansion = 1

    def __init__(self, args, device, batchNorm=True):
        super(Let_Flownet_EvtCount, self).__init__()
        ...
        # (dt * 10 * 1e-3) / (5 * dt) perfectly simplifies to a constant 2 milliseconds per bin
        time_step = 2e-3
...
def let_flownet_evtcount(args, device, data=None):
    model = Let_Flownet_EvtCount(args, device)
